...
def get_ma_phieu(user):
    con = get_connection()
    cur = con.cursor()
    cur.execute(f"select ma_dethi,account,status from tra_loi_riasec group by ma_dethi,account,status having account=%s and status is null limit 1",(user,))
    row = cur.fetchall()
    con.commit()
    cur.close()
    con.close()
...
def application(environment, start_response):
    ...
    if 'username' not in session:
        ...
        if int(ps[0][2]) > 0:
            ...
            loadurl = f"""'{load}/load_ket_qua_trac_nghiem_riasec'"""
            page = ""
            page += head
            ...
            page += f"""
                ...
                <p> Account : {user} </p>
                <p> Hai lĩnh vực bạn phù hợp với bạn nhất </p>
                <br />
        <p>
        ...
        <script>
        var display = {display};
        var colu = ["nhom_cau_hoi","count","khai_niem"];
    var $$ = function(id) {{
    ...
    hot = new Handsontable($container[0], {{
        ...
        autoWrapRow: true,
        rowHeaders: true,
        colHeaders: ["Nhóm câu hỏi","Đếm","Khái niệm"],

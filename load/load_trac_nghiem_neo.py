...
def count_rows(user):
    con = get_conection()
    cur = con.cursor()
    cur.execute(f"""select count(*) from tra_loi_neo where account=%s""", (user,))
...
def get_rows(display, start_w, user):
    ...
    cur = con.cursor()
    cur.execute(
        f"""select b.id,b.ma_cau_hoi,a.cau_hoi,b.tra_loi from (select ma_cau_hoi,cau_hoi from cau_hoi_neo) as a left outer join
(select id, ma_cau_hoi,tra_loi from tra_loi_neo where account=%s) as b

...
def count_rows(user):
    con = get_conection()
    cur = con.cursor()
    cur.execute(f"""select count(*) from (select nhom_cau_hoi,count(*) from (select ma_cau_hoi,nhom_cau_hoi from cau_hoi_riasec) as a left outer join
(select ma_cau_hoi, tra_loi from tra_loi_riasec where account=%s  and (tra_loi is not null or tra_loi !='')) as b on a.ma_cau_hoi = b.ma_cau_hoi
group by nhom_cau_hoi,tra_loi having tra_loi='true' order by count desc limit 2) as c left outer join
(select ten_nhom,khai_niem from nhom_cau_hoi) as d on c.nhom_cau_hoi = d.ten_nhom  """, (user,))
...
def get_rows(display, start_w, user):
    ...
    cur = con.cursor()
    cur.execute(
        f"""select c.nhom_cau_hoi, c.count,d.khai_niem from (select nhom_cau_hoi,count(*) from (select ma_cau_hoi,nhom_cau_hoi from cau_hoi_riasec) as a left outer join
(select ma_cau_hoi, tra_loi from tra_loi_riasec where account=%s  and (tra_loi is not null or tra_loi !='')) as b on a.ma_cau_hoi = b.ma_cau_hoi
group by nhom_cau_hoi,tra_loi having tra_loi='true' order by count desc limit 2) as c left outer join
(select ten_nhom,khai_niem from nhom_cau_hoi) as d on c.nhom_cau_hoi = d.ten_nhom order by c.count desc limit {display} offset {start_w}""",
        (user,))
    rows = cur.fetchall()
    con.commit()
    cur.close()
...
def application(environment, start_response):
    ...
    if 'username' not in session:
        ...
        if ps[0][2] > 0:
            ...
            page = '{"product":'
            objects_list = []
            cols = ["nhom_cau_hoi", "count", "khai_niem"]
